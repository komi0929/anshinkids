...
## 保護者からの新しい相談:
${question}
## 回答フォーマットの絶対ルール（Nani式マルチトーン）:
単一の回答ではなく、質問の意図（誰に伝えるか、どのように対処するか）に応じて、**必ず以下の3つのバリエーション（選択肢）を提示**してください。
1. 📝 **【そのまま使える簡潔な表現】** （連絡帳やLINE用）
2. 🗣️ **【丁寧に伝える表現】** （面談や初対面のお店の人用）
3. ⚠️ **【緊急・念押し表現】** （絶対に誤解を防ぎたい場合用）
※それぞれの表現に短い解説を添え、保護者が一番状況に合うものを選んでそのまま「コピーボタン」で使えるように工夫してください。

上記のWikiデータのみを参照し、温かく寄り添いながら、明確な3つの選択肢を提示して回答してください。`;
