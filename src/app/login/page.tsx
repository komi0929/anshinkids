  ...
  async function handleLineLogin() {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "kakao" as "google", // LINE is configured as custom OIDC; fallback to direct URL below
        options: {
          redirectTo: `${window.location.origin}/auth/callback`,
        },
      });

      // LINE Login via Supabase custom OIDC
      // Supabase doesn't have a built-in LINE provider, so we use the manual OAuth flow
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-[var(--color-bg)]">
      <div className="w-full max-w-sm">
        ...
        {/* Info box */}
        <div className="mt-8 p-4 rounded-xl bg-[var(--color-surface-warm)] border border-[var(--color-border-light)]">
