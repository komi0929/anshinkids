    ...
    <div className="fade-in pb-12 w-full max-w-2xl mx-auto">
      ...
      <div className="page-header border-b border-[var(--color-border-light)] bg-[var(--color-surface)]/95 backdrop-blur-sm sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[17px] font-extrabold text-[var(--color-text)] tracking-tight">あんしんキッズ</h1>
            <p className="text-[11px] font-bold text-[var(--color-text-secondary)]">みんなで育てる知恵袋コミュニティ</p>
          </div>
        </div>
      </div>
        ...
        {tier === "guest" && (
          <div className="rounded-3xl bg-gradient-to-br from-[var(--color-primary-light)] to-[var(--color-success-light)] p-6 relative overflow-hidden shadow-sm group slide-up border border-[var(--color-border-light)] cursor-pointer" onClick={() => window.location.href='/login'}>
            ...
            <div className="relative z-10 text-center">
               ...
               <h2 className="text-[18px] font-extrabold text-[var(--color-text)] mb-2 leading-tight">
                 ようこそ！<br/>子どものアレルギーの知恵を、<br/>みんなで共有しましょう
               </h2>
               <p className="text-[12px] text-[var(--color-text-secondary)] font-medium mb-5 leading-relaxed">
                 ログインすると、アレルゲン情報に合わせた「あなた専用の知恵袋」が届き、みんなの投稿にリアクションできるようになります。
               </p>
               <button className="btn-primary w-full max-w-xs mx-auto text-[14px]">
                 30秒で始める（無料登録）
               </button>
            </div>
          </div>
        ...
        {tier !== "guest" && (
          <div className="slide-up relative">
             <div className="flex items-center justify-between mb-4">
               <div>
                  <h2 className="text-[20px] font-extrabold text-[var(--color-text)]">こんにちは 👋</h2>
                  <p className="text-[12px] text-[var(--color-text-secondary)] font-medium">
                    {tier === "contributor" ? "今日もコミュニティの資産作りにご協力ありがとうございます" : "プロフィール設定が完了しました。知恵袋を活用しましょう"}
                  </p>
               </div>
             </div>
               ...
               <div className="grid grid-cols-2 gap-3 mb-2">
                 <Link href="/mypage" className="p-4 rounded-3xl bg-gradient-to-br from-[var(--color-surface-warm)] to-green-50 border border-green-100 flex flex-col justify-between h-24 shadow-sm relative overflow-hidden group hover:border-[var(--color-success)]/40 transition-colors cursor-pointer">
                   ...
                   <div className="flex items-end gap-1.5">
                     <span className="text-[28px] font-extrabold text-[var(--color-success-deep)] leading-none">{impact.articlesHelped}</span>
        ...
        {tier === "reader" && (
           <div className="card-elevated p-5 flex items-start gap-4 slide-up border-l-4 border-l-[var(--color-accent)] cursor-pointer group hover:bg-[var(--color-surface-warm)] transition-colors" onClick={() => window.location.href='/talk'}>
             ...
             <div className="flex-1">
               <h3 className="text-[15px] font-bold text-[var(--color-text)] mb-1 group-hover:text-[var(--color-accent)] transition-colors">最初の一歩を踏み出そう</h3>
          ...
          <div className="slide-up">
            <h3 className="text-[15px] font-extrabold text-[var(--color-text)] mb-3 flex items-center justify-between">
              <span className="flex items-center gap-2">
                 ...
                 {(tier === "guest" || !recommended.isPersonalized) && <span className="text-[10px] text-[var(--color-subtle)] font-normal ml-1 border rounded px-1.5">全体人気</span>}
              </span>
              <Link href="/wiki" className="text-[12px] text-[var(--color-subtle)] hover:text-[var(--color-primary)] font-bold flex items-center">
            ...
            <div className="space-y-3">
              {recommended.data.map(item => (
                <Link key={item.id} href={`/wiki/${item.slug}`} className="block p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] hover:border-[var(--color-primary)] transition-all group">
                  <div className="flex items-center gap-2.5 mb-2">
                     ...
                     {item.allergen_tags && item.allergen_tags.length > 0 && (
                       <span className="text-[10px] font-bold text-[var(--color-text-secondary)] opacity-80 flex gap-1 line-clamp-1 truncate">
                         {item.allergen_tags.slice(0, 2).map((t: string) => `#${t}`).join(" ")}
                       </span>
                     )}
                  </div>
                  <h4 className="font-bold text-[14px] text-[var(--color-text)] leading-snug mb-2 group-hover:text-[var(--color-primary)] transition-colors">
                    {item.title}
                  </h4>
        ...
        {trending.length > 0 && (
          <div className="slide-up" style={{ animationDelay: '100ms' }}>
            <h3 className="text-[15px] font-extrabold text-[var(--color-text)] mb-3 flex items-center justify-between">
              ...
              <Link href="/talk" className="text-[12px] text-[var(--color-subtle)] hover:text-[var(--color-primary)] font-bold flex items-center">
                声を聞く <ChevronRight className="w-3.5 h-3.5" />
              </Link>
            </h3>
            <div className="grid gap-3">
              {trending.map((room) => (
                <Link key={room.roomId} href={`/talk/${room.slug}`} className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--color-surface-warm)] border border-[var(--color-border-light)] hover:border-[var(--color-border)] shadow-sm hover:shadow-md transition-all group">
                  <div className="w-12 h-12 rounded-xl bg-white border border-[var(--color-border-light)] shadow-sm flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">
                    {room.icon_emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-[14px] text-[var(--color-text)] truncate">{room.name}</h4>
                    <p className="text-[12px] text-[var(--color-text-secondary)] mt-1 flex items-center gap-2">
                       <span className="flex items-center gap-1"><MessageCircle className="w-3.5 h-3.5" />{room.messageCount}件の投稿</span>
                       {room.thanksTotal > 0 && <span className="flex items-center gap-1 text-[var(--color-heart)]"><Heart className="w-3.5 h-3.5" />{room.thanksTotal}件の感謝</span>}
                    </p>
                  </div>
        ...
        {bookmarks.length > 0 && (
          <div className="slide-up pb-4" style={{ animationDelay: '150ms' }}>
            <h3 className="text-[15px] font-extrabold text-[var(--color-text)] mb-3 flex items-center justify-between">
              ...
              <Link href="/mypage" className="text-[12px] text-[var(--color-subtle)] font-bold flex items-center hover:text-[var(--color-primary)]">
                すべて見る <ChevronRight className="w-3.5 h-3.5" />
              </Link>
            </h3>
            <div className="space-y-3">
              {bookmarks.slice(0, 2).map((bm) => (
                <Link key={bm.id} href={`/wiki/${bm.wiki_entries.slug}`} className="block p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] hover:border-[var(--color-primary)] hover:shadow-sm transition-all group">
                   <p className="text-[10px] text-[var(--color-subtle)] mb-1 flex items-center gap-1">
                     <BookOpen className="w-3 h-3" /> {bm.wiki_entries.category} / {bm.wiki_entries.title}
                   </p>
                   <h4 className="text-[13px] font-bold text-[var(--color-text)] mb-1 leading-tight group-hover:text-[var(--color-primary)] transition-colors">{bm.snippet_title}</h4>
                   <p className="text-[12px] text-[var(--color-text-secondary)] line-clamp-1">{bm.snippet_content}</p>
                </Link>
              ))}
            </div>
          </div>
