...
export default function WikiClient({ initialEntries }: WikiClientProps) {
 ...
 useEffect(() => {
 // Skip the first render since we have initialEntries
 ...
 className={`px-4 py-2 rounded-full text-[13px] font-bold transition-all ${sortBy === opt.key ? "bg-[var(--color-text)] text-white " : "bg-white text-[var(--color-text-secondary)] "}`}
 >
 {opt.label}
 </button>
 ))}
 ...
 className={`flex-shrink-0 flex items-center justify-center w-11 h-11 rounded-full transition-all ${showFilters || selectedAllergens.length > 0 ? "bg-[var(--color-primary)] text-white " : "bg-white text-[var(--color-text-secondary)] "}`}
 >
 ...
 className={`flex-shrink-0 px-4 py-2.5 rounded-full text-[14px] font-bold transition-all ${selectedCategory === cat ? "bg-[var(--color-primary)] text-white transform -translate-y-0.5" : "bg-white text-[var(--color-text-secondary)] "}`}
 >
 ...
 className={`px-4 py-2 rounded-full text-[13px] font-bold transition-all ${selectedAllergens.includes(allergen.label) ? "bg-[var(--color-primary)] text-white scale-[1.02]" : "bg-[var(--color-surface-soft)] text-[var(--color-text-secondary)]"}`}
 >
 {allergen.label}
 </button>
 ))}
 ...
 {isLoading ? (
 [1, 2, 3].map((i) => (
 <div key={i} className="bg-white rounded-2xl p-6 border-[1.5px] border-[var(--color-border)] slide-up">
 ...
 <div className="w-16 h-16 mx-auto bg-white rounded-2xl flex items-center justify-center mb-4">
 ...
 <Link href="/talk" prefetch={true} className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-full bg-white border-[1.5px] border-[var(--color-border)] text-[13px] font-bold text-[var(--color-primary)] hover:bg-[var(--color-surface-soft)] transition-all">
 ...
 prefetch={true}
 className="block relative bg-white rounded-2xl p-6 border border-transparent hover:border-[var(--color-primary)]/20 hover:shadow-[0_8px_32px_rgb(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300"
 >
