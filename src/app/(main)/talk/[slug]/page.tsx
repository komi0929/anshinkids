...
export default function TalkRoomPage() {
  ...
  async function loadRelatedWiki(roomId: string) {
    ...
    if (result.success && result.data)
      setRelatedWiki(result.data as RelatedWiki[]);
  }
  ...
  function handleMessageChange(text: string) {
    ...
    if (text.length > 10) {
      ...
      if (safety.hasDangerousAdvice) {
        setSafetyWarning(
          "⚠️ 医療的な判断を含む可能性があります。「うちの場合は」のような体験談の形で共有していただけると安心です。",
        );
      } else if (safety.hasNegativeContent) {
        setSafetyWarning(
          "💚 あんしんキッズは共感と応援の場です。言葉を少しやさしくしてみませんか？",
        );
      }
    }
  }
  ...
  async function handleSend(content?: string) {
    ...
    if (safety.hasDangerousAdvice) {
      setSafetyWarning(
        "⚠️ この投稿には医療判断に関わる内容が含まれている可能性があります。「うちの場合は〜でした」のように体験談として書き直してみてください。",
      );
      return;
    }
    ...
    if (result.success) {
      ...
      const stored = parseInt(
        typeof window !== "undefined"
          ? localStorage.getItem("anshin_post_count") || "0"
          : "0",
      );
      ...
      if (newTotal === 1) {
        setTimeout(() => {
          setMilestoneToast(
            "🎉 初めての投稿！ あなたの声がだれかの助けになります",
          );
          setTimeout(() => setMilestoneToast(null), 5000);
        }, 4500);
      } else if (newTotal === 5) {
        setTimeout(() => {
          setMilestoneToast("🌿 5回目の投稿！ 知恵袋がどんどん充実しています");
          setTimeout(() => setMilestoneToast(null), 5000);
        }, 4500);
      } else if (newTotal === 10) {
        setTimeout(() => {
          setMilestoneToast(
            "✨ 10回目！ たくさんの親子があなたの知恵に出会っています",
          );
          setTimeout(() => setMilestoneToast(null), 5000);
        }, 4500);
      } else if (newTotal === 25) {
  ...
  async function handleThanks(messageId: string, event?: React.MouseEvent) {
    ...
    if (thankedIds.has(messageId)) {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === messageId ? { ...m, thanks_count: m.thanks_count + 1 } : m,
        ),
      );
  ...
  function getAvatarColor(name: string) {
    const colors = [
      "from-[#7FA77A] to-[#5C8B56]",
      "from-[#B8956A] to-[#9A7A52]",
      "from-[#8B9EBF] to-[#6A7FA0]",
      "from-[#C2917A] to-[#A87060]",
      "from-[#9BB88F] to-[#7A9E6E]",
      "from-[#B8A07A] to-[#9A8560]",
    ];
  ...
  const renderedMessages = useMemo(() => {
    ...
    return messages.map((msg) => {
      ...
      return (
        <div
          key={msg.id}
          className={`fade-in ${msg.is_optimistic ? "animate-pulse" : ""} ${opacityClass}`}
        >
          {msg.is_system_bot ? (
            ...
            <div className="card p-4">
              <div className="flex items-start gap-3">
                <div
                  className={`w-9 h-9 rounded-full bg-gradient-to-br ${getAvatarColor(msg.user_id)} flex items-center justify-center flex-shrink-0 shadow-sm relative overflow-hidden`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[14px] leading-[1.8] text-[var(--color-text)] whitespace-pre-wrap">
                    {msg.content}
                  </p>
                  ...
                  <div className="flex items-center gap-2 mt-3 flex-wrap">
                    {!isMyMessage && (
                      ...
                      >
                        ...
                        {msg.thanks_count > 0 && (
                          <span className="font-bold text-[var(--color-heart)]">
                            {msg.thanks_count}
                          </span>
                        )}
                      </motion.button>
                    ...
                    {isMyMessage && (
                      <div className="flex items-center gap-1.5 ml-1">
                        ...
                        <button
                          ...
                          className="p-1.5 text-[var(--color-muted)] hover:text-[var(--color-danger)] transition-colors rounded-full hover:bg-[var(--color-danger-light)]0"
                          aria-label="投稿を削除"
                        >
  ...
  return (
    <div className="flex flex-col h-[100dvh]">
      ...
      <div className="px-4 py-3 flex items-center gap-3 border-b border-[var(--color-border-light)] bg-[var(--color-surface)]/95 backdrop-blur-sm sticky top-0 z-40">
        ...
        <div className="flex items-center gap-2.5 flex-1">
          ...
          <div>
            ...
            >
              {roomInfo?.description || "体験や情報を気軽にシェア"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        {showEmergency && (
          <div
            className="p-4 rounded-2xl bg-[var(--color-danger-light)]0 border-2 border-red-300 mb-3 slide-up"
            id="emergency-banner"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-5 h-5 text-[var(--color-danger)]" />
                <h3 className="text-[14px] font-extrabold text-red-700 break-keep text-balance">
            ...
            <div className="space-y-1.5 mb-3">
              {EMERGENCY_GUIDANCE.steps.map((step, i) => (
                <p key={i} className="text-[12px] text-red-700 font-medium">
                  {step}
                </p>
              ))}
            </div>
            <p className="text-[11px] text-[var(--color-danger)] font-bold">
              {EMERGENCY_GUIDANCE.important}
            </p>
            <a
              href="tel:119"
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 text-[10px] text-[var(--color-subtle)]">
                <span className="flex items-center gap-1 bg-[var(--color-surface)] px-2 py-0.5 rounded-full">
                  <Clock className="w-2.5 h-2.5" /> 発言 → 自動消去
                </span>
                <ArrowRight className="w-2.5 h-2.5 text-[var(--color-muted)]" />
                <span className="flex items-center gap-1 bg-[var(--color-success-light)] px-2 py-0.5 rounded-full text-[var(--color-success-deep)]">
                  <BookOpen className="w-2.5 h-2.5" /> 一次情報 →
                  知恵袋に永久保存
                </span>
              </div>
            </div>
          ...
          <div className="mb-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[11px] font-bold text-[var(--color-text)]">
                このテーマの知恵袋 ({relatedWiki.length}件)
              </span>
            </div>
            <div className="flex gap-2 overflow-x-auto pb-1.5 -mx-1 px-1">
              {relatedWiki.map((wiki) => (
                <Link
                  key={wiki.id}
                  href={`/wiki/${wiki.slug}`}
                  className="flex-shrink-0 px-3.5 py-2.5 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-success)]/15 hover:border-[var(--color-success)]/40 transition-all shadow-sm group max-w-[200px]"
                >
                  <p className="text-[11px] font-bold text-[var(--color-text)] truncate group-hover:text-[var(--color-success-deep)] transition-colors">
                    📖 {wiki.title}
                  </p>
                  <p className="text-[9px] text-[var(--color-muted)] mt-0.5">
                    {wiki.source_count}件の体験から
                  </p>
                </Link>
              ))}
            </div>
        ...
        {messages.length === 0 && !isLoading && (
          <div className="mb-6 fade-in">
            <div className="text-center mb-6">
              ...
              <h2 className="text-[16px] font-extrabold text-[var(--color-text)] break-keep text-balance px-4">
                「{roomInfo?.name}」について話そう
              </h2>
              <p className="text-[12px] text-[var(--color-subtle)] mt-2 leading-relaxed">
              ...
              {isLoadingPrompts ? (
                ...
                prompts.map((prompt, i) => (
                  <button
                    ...
                    className="w-full text-left p-4 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] hover:border-[var(--color-primary)] hover:bg-[var(--color-surface-warm)] transition-all group stagger-item"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[var(--color-primary)]/10 to-[var(--color-primary)]/5 flex items-center justify-center flex-shrink-0">
            ...
            {!isLoadingPrompts && (
              <button
                ...
                className="flex items-center gap-1.5 mx-auto mt-4 px-4 py-2 rounded-full text-[11px] text-[var(--color-subtle)] hover:text-[var(--color-primary)] hover:bg-[var(--color-surface-warm)] transition-all"
                id="refresh-prompts"
              >
        ...
        {messages.length > 0 && (
          <div className="mb-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <button
                onClick={refreshPrompts}
                className="ml-auto text-[var(--color-subtle)] hover:text-[var(--color-primary)] transition-colors p-1"
              >
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">
              {isLoadingPrompts ? (
                <div className="shimmer h-8 w-40 rounded-full flex-shrink-0" />
              ) : (
                prompts.map((prompt, i) => (
                  <button
                    key={i}
                    onClick={() => handlePromptClick(prompt)}
                    className="flex-shrink-0 px-3.5 py-2 rounded-full bg-[var(--color-surface)] border border-[var(--color-border)] text-[11px] text-[var(--color-text-secondary)] hover:border-[var(--color-primary)] hover:bg-[var(--color-primary)]/5 transition-all whitespace-nowrap shadow-sm"
                  >
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-white/90 leading-snug mt-0.5">
                  {impactMessage ||
                    assetMessages[postCount % assetMessages.length]}
                </p>
              </div>
            </div>
          </div>
      ...
      {/* === F3: Safety Warning === */}
      {safetyWarning && (
        <div className="px-4 py-2.5 bg-[var(--color-warning-light)]0 border-t border-[var(--color-warning)]/30">
          <div className="flex items-center gap-2 max-w-lg mx-auto">
            <ShieldCheck className="w-4 h-4 text-[var(--color-warning)] flex-shrink-0" />
            <p className="text-[11px] text-[var(--color-warning)] leading-snug flex-1">
              {safetyWarning}
            </p>
            <button
              onClick={() => setSafetyWarning(null)}
              className="text-amber-400 hover:text-[var(--color-warning)]"
            >
        ...
        <div className="max-w-lg mx-auto">
          <div className="flex gap-3 items-end">
            <textarea
              ...
              className="input-field flex-1 resize-none max-h-32 transition-none"
              rows={1}
              id="message-input"
              onInput={(e) => {
            ...
            <motion.button
              ...
              onClick={() => handleSend()}
              disabled={
                !newMessage.trim() || isSending || authError || !!safetyWarning
              }
              className="btn-primary flex-shrink-0 mb-[2px]"
              id="message-send"
            >
          ...
          >
            💬 書き込みは一定期間後に消えますが、AIが知恵袋に知識として残します
          </p>
        </div>
      </div>
        ...
        >
          ...
          <div className="relative w-full max-w-md mx-auto bg-[var(--color-surface)] rounded-3xl shadow-2xl slide-up max-h-[85vh] sm:max-h-[80vh] overflow-hidden flex flex-col mb-[env(safe-area-inset-bottom,0px)]">
            <div className="p-5 flex justify-between items-center border-b border-[var(--color-border-light)] bg-white sticky top-0 z-10 shrink-0 rounded-t-3xl">
              <h2 className="text-[17px] font-extrabold text-[var(--color-text)] flex-1 text-center pr-6 break-keep text-balance">
                {COMMUNITY_GUIDELINES.title}
              </h2>
              <button
                onClick={() => setShowGuidelines(false)}
                className="w-8 h-8 rounded-full bg-[var(--color-surface-warm)] flex items-center justify-center text-[var(--color-subtle)] hover:bg-gray-200 absolute right-4"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto pb-24">
              <div className="space-y-3 mb-5">
                {COMMUNITY_GUIDELINES.rules.map((rule, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 p-3.5 rounded-2xl bg-[var(--color-surface-warm)]"
                  >
                    <span className="text-xl">{rule.emoji}</span>
                    <div>
                      <h4 className="text-[13px] font-bold text-[var(--color-text)] break-keep text-balance">
                        {rule.title}
                      </h4>
                      <p className="text-[11px] text-[var(--color-subtle)] mt-0.5 leading-relaxed">
                        {rule.description}
                      </p>
                    </div>
                  </div>
              ...
              <button
                ...
                className="btn-primary w-full flex items-center justify-center gap-2 py-3.5 shadow-md"
